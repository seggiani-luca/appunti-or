...
\begin{enumerate}
	\item $P_{ij} = \emptyset$ significa che per un certo nodo $P_{ij}$ possiamo tagliare tutti i problemi che istanziano le $i$ variabili di $P_{ij}$, ergo tutti i suoi nodi figl;
	\item Calcolo di $v_S(P_{ij})$. Se $v_S(P_{ij}) < v_I(P)$ figlio, allora posso scartare il sottoalbero.
\end{enumerate}
